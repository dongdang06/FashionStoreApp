...
public class PhieuXuatTraDAO {
	...
	private PhieuXuatTra getById(Connection conn, String maPhieuTra) throws SQLException {
		try (PreparedStatement stmt = conn.prepareStatement(
				"SELECT MaPhieuTra, MaNCC, MaNV, NgayTra, LyDo FROM PHIEUXUATTRA WHERE MaPhieuTra = ?")) {
			stmt.setString(1, maPhieuTra);
			try (ResultSet rs = stmt.executeQuery()) {
				if (!rs.next())
					return null;
	...
	private List<ChiTietPhieuXuat> getDetails(Connection conn, String maPhieuTra) throws SQLException {
		...
		try (PreparedStatement stmt = conn.prepareStatement(
				"SELECT MaPhieuTra, MaBienThe, SoLuong FROM CHITIETPHIEUXUATTRA WHERE MaPhieuTra = ? ORDER BY MaBienThe")) {
			stmt.setString(1, maPhieuTra);
			try (ResultSet rs = stmt.executeQuery()) {
				while (rs.next())
					details.add(new ChiTietPhieuXuat(rs.getString("MaPhieuTra"), rs.getString("MaBienThe"),
							rs.getInt("SoLuong")));
			}
		}
		return details;
	...
	private void updateHeader(Connection conn, PhieuXuatTra r) throws SQLException {
		try (PreparedStatement stmt = conn.prepareStatement(
				"UPDATE PHIEUXUATTRA SET MaNCC = ?, MaNV = ?, NgayTra = ?, LyDo = ? WHERE MaPhieuTra = ?")) {
			stmt.setString(1, r.getMaNCC());
			stmt.setString(2, r.getMaNV());
			stmt.setDate(3, new java.sql.Date(r.getNgayTra().getTime()));
			stmt.setString(4, r.getLyDo());
			stmt.setString(5, r.getMaPhieuTra());
			stmt.executeUpdate();
		}
	}
	...
	private void validateXuatStockAfterChange(Connection conn, List<ChiTietPhieuXuat> oldDetails,
		...
		for (ChiTietPhieuXuat d : oldDetails)
			oldMap.merge(d.getMaBienThe(), d.getSoLuong(), Integer::sum);
		for (ChiTietPhieuXuat d : newDetails)
			newMap.merge(d.getMaBienThe(), d.getSoLuong(), Integer::sum);
		...
		for (String maBT : variants) {
			...
			if (finalStock < 0)
				throw new IllegalArgumentException("So luong ton kho khong du de xuat tra bien the " + maBT + ".");
		}
	}
	...
	private void validateReturn(PhieuXuatTra r) {
		...
		if (r.getNgayTra() == null)
			r.setNgayTra(new java.util.Date());
		...
		Set<String> variants = new HashSet<>();
		for (ChiTietPhieuXuat d : details) {
			d.setMaPhieuTra(r.getMaPhieuTra());
